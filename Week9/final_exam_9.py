...
Problem 9: Regularized lin reg for cassification.
...
for digit in range(num_of_labels):
    ...
    ###Without transform, E_in
    train_matrix = make_matrix(train_data)
    weights = regularization_calc_weight_vec(train_matrix, lam, one_v_all_labels)
    curr_E_in = calc_E(train_matrix, one_v_all_labels, weights)
    e_in_notrans.append(curr_E_in)

    ###Without transform, E_out    
    test_matrix = make_matrix(test_data)
    curr_E_out = calc_E(test_matrix, one_v_all_labels_test, weights)
    e_out_notrans.append(curr_E_out)
    

print "e_out_trans", e_out_trans, "\n"
print "e_out_notrans",e_out_notrans, "\n"
for i in range(len(e_out_trans)):
    print "digit", i, (e_out_trans[i] - e_out_notrans[i]) / e_out_notrans[i]
