...
Problem 17: 
Regualar RBF,
gamma = 1.5 versus 2, K = 9 clusters.
...
How E_in and E_out react as gamme from 1.5 to 2
...
nclust = 9
successful_runs = 0
result_tracker = [0,0,0,0,0] #corresponds to five ans choices
for i in range(nosims):
    ...
    for j in range(nclust):
        init_clusters.append(gen_pt())
    init_clusters = np.asarray(init_clusters)
    
    #train regular RBF model:
    #use kmeans (lloyd's algo) to find centers then pseudoinv to calc weight
    km = cluster.KMeans(n_clusters = nclust, init = init_clusters)
    km.fit(train_data)
